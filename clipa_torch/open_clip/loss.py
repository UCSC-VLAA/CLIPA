...
try:
    ...
    import torch_xla
    _HAS_XLA = True
    from .all_gather_xla import all_gather as xlaf_all_gather
    ...
    if use_horovod:
        ...
        if use_xla():
            if gather_with_grad:
                ...
                all_image_features = xm.all_gather(image_features)
                all_text_features = xm.all_gather(text_features)
                if not local_loss:
                    # ensure grads for local rank when all_* features don't have a gradient
                    all_image_features[rank*batch_size:(rank+1)*batch_size] = image_features
