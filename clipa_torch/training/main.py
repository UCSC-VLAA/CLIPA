    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if any(v in data for v in ('val', 'imagenet-val', 'imagenet-v2')):
            evaluate(model=model,
                     ...
                     step=data['train'].dataloader.num_batches // args.accum_freq * completed_epoch,
                     args=args,
                     should_zero_eval=args.zeroshot_frequency != 0 and
                                      ((completed_epoch % args.zeroshot_frequency) == 0 or completed_epoch == args.epochs),
                     should_val=args.val_frequency != 0 and
                                ((completed_epoch % args.val_frequency) == 0 or completed_epoch == args.epochs),
                     tb_writer=writer)
        ...
        if args.save_logs:
            ...
            if completed_epoch == args.epochs or (
                ...
                if use_xla():
                    checkpoint_dict = state_dict_to_cpu(checkpoint_dict)
                torch.save(
                    checkpoint_dict,
