...
class ReaderTfds:
    ...
    def __init__(
        ...
        assert self.input_name in ['image', 'jpg'] # need to adapt to decoder
        ...
        if name is not None:
            ...
            self.builder = tfds.builder_from_directory(root)
        ...
        self.single_replica = single_replica
        self.dist_rank = 0
        ...
        self.epoch_count = SharedEpoch(epoch)
        ...
        # self.reinit_each_iter = False
        if self.is_training:
            self.reinit_each_iter = REINIT_EACH_ITER
            logging.info(f'using REINIT_EACH_ITER = {REINIT_EACH_ITER} in training!!!')
        else:
            self.reinit_each_iter = False

    def set_epoch(self, count):
        # self.epoch_count.value = count
        self.epoch_count.set_value(count)
    ...
    def _lazy_init(self):
        ...
        if worker_info is not None:
            ...
            self.num_workers = worker_info.num_workers if worker_info.num_workers != 0 else 1
            self.global_num_workers = self.dist_num_replicas * self.num_workers
            global_worker_id = self.dist_rank * self.num_workers + worker_info.id

            # TODO: add code for reassigning shards each epoch
            """ Data sharding
        ...
        read_config = tfds.ReadConfig(
            shuffle_seed=self.common_seed + self.epoch_count.get_value(),
        ...
        ds = self.builder.as_dataset(
            ...
            decoders=dict(image=decode_example()) if self.input_name == 'image' else dict(jpg=decode_example()),
        ...
        if self.is_training:
            ds = ds.shuffle(min(self.num_samples, self.shuffle_size), seed=self.worker_seed)
    ...
    def _num_samples_per_worker(self):
        ...
        if self.is_training and self.train_num_samples is not None:
            assert int(self.train_num_samples // max(self.global_num_workers, self.dist_num_replicas)) <= num_worker_samples
            return int(self.train_num_samples // max(self.global_num_workers, self.dist_num_replicas))
