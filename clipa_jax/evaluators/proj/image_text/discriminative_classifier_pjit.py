...
from evaluators.proj.image_text import prompt_engineering
from transforms import ops_general  # pylint: disable=unused-import
from transforms import ops_image  # pylint: disable=unused-import
...
import os
#os.makedirs('~/CLIPA/clipa_jax/memory_profile', exist_ok=True)
...
def _split_and_batch(dataset_name, data_dir, class_names, batch_size, split,
    ...
    assert not batch_size % jax.process_count(), (
        f"batch_size={batch_size} % jax.device_count()={jax.device_count()}")
    builder = tfds.builder(dataset_name, data_dir=data_dir)
    batch_dims = [ batch_size // jax.process_count()
    ]
...
class Evaluator:
    ...
    def __init__(self,
        ...
        def count_correct(
            ...
            zimg, _, _ = predict_fn(params, {"image":image},  None)
            ...
            matching = (best_txt == labels)
            correct = jnp.where(
                mask, (matching > 0).astype(
                    jnp.int32), 0).sum()
            correct = (correct).sum()
            return correct, zimg

        self._embed_texts_p = partitioner.partition(embed_texts,
                                                    in_axis_resources=(
                                                    train_state_axes, partitioner.data_partition_spec),
                                                    out_axis_resources=(None),
                                                    donate_argnums = (0,)
                                                    )
        self._count_correct_p = partitioner.partition(count_correct,
                                                      in_axis_resources=(
                                                          train_state_axes, None, partitioner.data_partition_spec, partitioner.data_partition_spec,
                                                          partitioner.data_partition_spec, None),
                                                      out_axis_resources=(None, None)
                                                      )


        self._count_p =   partitioner.partition(lambda mask: mask.sum().sum(),
                                                      in_axis_resources=(
        ...
        logging.info(
            "Initialized evaluator in %.1f seconds",
            time.monotonic() - t0)
        from jax.experimental import multihost_utils
        self.batch_jit_fn =  lambda x: multihost_utils.host_local_array_to_global_array(
    ...
    def _embed_texts(self, params, dataset_name):
        ...
        for batch in self.datasets[dataset_name]["texts"]:
            ...
            batch = jax.tree_map(lambda x: self.batch_jit_fn(np.asarray(memoryview(x))), batch)
            ns.append(self._count_p(batch['mask']))
            embeddings.append(self._embed_texts_p(params, batch["labels"]))
            for name in data:
                data[name].append(batch[name])

            temp_ns = jax.experimental.multihost_utils.process_allgather(ns, tiled=True)
            if len(ns) >= self._async_delay and temp_ns[-self._async_delay] == 0:
                break

            if self._embed_texts_p not in self._compiled:
                logging.info(
                    "Compiled text embeddings in %.1fs",
                    time.monotonic() - t0)
                t0 = time.monotonic()
        ...
        data  = {k: jax.experimental.multihost_utils.process_allgather(v, tiled=True) for k, v in data.items()}
        data = {k: jnp.concatenate(jnp.asarray(v), axis=0) if len(jnp.asarray(v).shape) >1 else jnp.asarray(v) for k, v in data.items()}
        ...
        logging.info("Embedded %s text in %d steps - ...%s",
                     dataset_name, len(ns), ns[-10:])
    ...
    def evaluate(self,
        ...
        for batch in self.datasets[dataset_name]["images"]:
            ...
            # ns[-1] allows us to tune additional steps vs. interleaved
            # processing.
            ns.append(self._count_p(batch['mask']))
            temp_ns = jax.experimental.multihost_utils.process_allgather(ns, tiled=True)
            if len(ns) >= self._async_delay and temp_ns[-self._async_delay] == 0:
        ...
        ns = np.array(ns)
        n = ns.sum()
        corrects = jax.experimental.multihost_utils.process_allgather(corrects, tiled=True)
        correct = jnp.array(corrects).sum()

        logging.info("Embedded %s image in %d steps - ...%s",
                     dataset_name, len(ns), ns[-10:])
