...
def rotate(image, degrees, replace):
    ...
    image = contrib_image.rotate(
        wrap(image), radians, interpolation='bilinear')
...
def translate_x(image, pixels, replace):
    ...
    image = contrib_image.translate(
        wrap(image), [-pixels, 0], interpolation='bilinear')
    return unwrap(image, replace)
...
def translate_y(image, pixels, replace):
    ...
    image = contrib_image.translate(
        wrap(image), [0, -pixels], interpolation='bilinear')
    return unwrap(image, replace)
...
def shear_x(image, level, replace):
    ...
    image = contrib_image.transform(
        wrap(image), [
            1., level, 0., 0., 1., 0., 0., 0.], interpolation='bilinear')
    return unwrap(image, replace)
...
def shear_y(image, level, replace):
    ...
    image = contrib_image.transform(
        wrap(image), [
            1., 0., 0., level, 1., 0., 0., 0.], interpolation='bilinear')
    return unwrap(image, replace)
...
def distort_image_with_randaugment(
    ...
    available_ops = [
        ...
        'ShearY',
        'TranslateX',
        # same as relative trans in timm (because timm use a global constant)
        'TranslateY',
        ...
        with tf.name_scope('randaug_layer_{}'.format(layer_num)):
            for (i, op_name) in enumerate(available_ops):
                ...
                prob = tf.random_uniform(
                    [], minval=0.2, maxval=0.8, dtype=tf.float32)

                func, _, args = _parse_policy_info(
                    op_name, prob, random_magnitude, replace_value, augmentation_hparams, increase=increase)
                image = tf.cond(
                    tf.equal(
                        i,
                        op_to_select),
                    lambda selected_func=func,
                    selected_args=args: selected_func(
                        image,
                        *selected_args),
                    lambda: image)
...
def check_zero_image(test_img, org_img):
    ...
    return tf.cond(
        tf.equal(
            tf.reduce_sum(aug_image_np),
            0),
        lambda: org_img,
        lambda: test_img)


def distort_image_with_randaugment_timm(
        image,
        num_layers,
        magnitude,
        increase=False,
    ...
    for layer_num in range(num_layers):
        ...
        random_magnitude = tf.random.normal(
            [], mean=magnitude, stddev=0.5)  # follow the timm

        with tf.name_scope('randaug_layer_{}'.format(layer_num)):
            for (i, op_name) in enumerate(available_ops):
                aug_prob = tf.random_uniform(
