...
def get_onehot(depth,
    ...
    def _onehot(data):
        ...
        labels = data[key]
        # both scatter and one_hot expect this
        labels = tf.cast(labels, tf.int64)
        if labels.shape.rank > 0 and multi:
            x = tf.scatter_nd(labels[:, None], tf.ones(
                tf.shape(labels)[0]), (depth,))
...
def get_rag_tensor():
    ...
    def rag_tensor(raw_tensor):
        # Note: Add one more dimension as `from_tensor` requires at least rank
        # 2.
