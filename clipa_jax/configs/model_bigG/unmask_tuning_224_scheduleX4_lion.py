...
def get_config(arg=None):
  ...
  arg = bvcc.parse_arg(
      arg,  res=224, runlocal=False, batchsize=32768,  token_len=32, txt='bert_base', img='G/14',
 ...
 # input section include augmentation
  ...
  config.total_steps = int(131072000 * 1 // arg.batchsize)  # seen_samples // batchsize to get the number of steps
  config.lr = 8e-8 * (arg.batchsize // 256)
  config.wd = 2
  #warmup_steps = int(26214400 // arg.batchsize) # seen_samples // batchsize to get the number of steps
  warmup_steps = 0
  config.schedule = [
      ('.*', dict(decay_type='cosine', warmup_steps=warmup_steps, min_lr=0, max_lr=8e-8 * (arg.batchsize // 256))),
  ]
  ...
  #config.partitioning.logical_axis_rules = #\
  #   [
  #     ("batch", "data"),
  #     ("vocab", "model"),
  #     ("mlp", "model"),
  ...
  config.wandb = dict(
      ...
      experiment=f'G14_224_mask30_32k_{arg.res}_{arg.token_len}_gap_sin2d_lr8e',
      entity='xianhangli'
  )
