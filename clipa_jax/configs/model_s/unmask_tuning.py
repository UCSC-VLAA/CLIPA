...
def get_config(arg=None):
  ...
  arg = bvcc.parse_arg(
      arg,  res=224, runlocal=False, token_len=32, txt='bert_base', img='S/16',
  ...
  config.wandb = dict(
      ...
      experiment=f'S16_32k_{arg.res}_{arg.token_len}_tok_sin2d_lr8e',
      entity='xianhangli'
  )
  ...
  config.input = {}
  config.input.data = dict(name='liaon-400m', split='full', data_dir='gs://celt-data-eu-west4/laion-400m')
  ...
  config.input.pp = pp_eval = (
     # f'resize(inkey="jpg", size={arg.res})|vgg_value_range'
 ...
 # dim = 768
  ...
  batch_factor = 2
  config.input.batch_size = int(1024 * 16 * batch_factor)

  config.total_steps = 4000 if not arg.runlocal else 1
  config.lr = 8e-7 * 64  * batch_factor # lr for 256
  config.wd = 0.2
  warmup_steps = int(1600//batch_factor)  # for 16k batch size  # max(int(0.03 * config.total_epochs), 100)
  config.schedule = [
      #('img/.*', None),  # Freezes image tower.
      ('.*', dict(decay_type='cosine', warmup_steps=warmup_steps, min_lr=0, max_lr=8e-7 * 64 * batch_factor)),
  ]
  ...
  #     **eval_common,
  #     'data': dict(name='imagenet2012', split=f'validation{sub}', data_dir= 'gs://celt-tfds-imagenet-eu'),
  #     'pp_fn': (
  #         f'decode(inkey="image")|resize({arg.res})|vgg_value_range'
  ...
  # config.evals.disclf.pp_img = f'resize({arg.res})|vgg_value_range'
  ...
  config.evals.disclf.pp_txt = tokenizer('texts')
  #config.evals.disclf.prompt_templates = 'clip_best'
  #config.evals.disclf.pp_txt = f'|clip_i1k_label_names|{tokenizer("labels")}'
  config.evals.disclf.type = 'proj.image_text.discriminative_classifier'
  config.evals.disclf.prefix = 'z/0shot/'
  config.evals.disclf.log_steps = eval_common['log_steps']
