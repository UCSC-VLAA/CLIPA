...
class FlaxOptimTrainState(flax.struct.PyTreeNode):
    ...
    def replace_optimizer(self, new_opt):
        return self.replace(_optimizer=self._optimizer.replace(optimizer_def=new_opt))

    def replace_params_axes(self, train_params_axes: VariableDict)-> "FlaxOptimTrainState":
        return self.replace(params_axes=train_params_axes)
