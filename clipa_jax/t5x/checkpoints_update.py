...
class Checkpointer(object):
  ...
  def restore(
      ...
      mask_init: bool = False) -> train_state_lib.TrainState:
  ...
  def _create_lazy_awaitable_array(
      ...
      restore_dtype: Optional[jnp.dtype], mask_init: bool=False) -> LazyAwaitableArray:
    ...
    Returns:
      LazyArray object.
    """
    mesh = self._partitioner._mesh # hack for old t5x
    ...
    for k in written_state_dict.keys():
      ...
      state_dict[k] = jax.tree_util.tree_map(
          functools.partial(
              ...
              ckpt_path=ckpt_path,
              restore_dtype=restore_dtype,
              mask_init=mask_init), restore_parameter_infos[k],
  ...
  if param_info.shape is not None:
    ...
    if ts_spec_arr_shape != param_info.shape:
      if not  mask_init:
          ...
          logging.info((f'Shape of `{param_info.name}` in checkpoint '
                       f'{ts_spec_arr_shape} does not match expected '
                       f'{param_info.shape}. You should re-init them'))
